...
public class SingleChoiceValidator implements ConstraintValidator<SingleChoiceConstraint, QuestionDTO> {
    @Override
    public boolean isValid(QuestionDTO dto, ConstraintValidatorContext context) {
        if (dto.getType() == null || !dto.getType().equals(QuestionType.SINGLE)) {
            return true; // 不是单选题，不需要验证
        }
        if (dto.getChoices() == null || dto.getChoices().isEmpty()) {
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("单选题必须包含选项")
        ...
        if (correctCount != 1) {
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("单选题必须有且仅有一个正确选项")
                    .addPropertyNode("choices")
                    .addConstraintViolation();
            return false;
        }
